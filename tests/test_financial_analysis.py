...
def test_calculate_technical_indicators(analysis):
    analysis.df = pd.DataFrame({
        "Close": [100, 102, 104, 106, 108, 110, 112, 114, 116, 118, 120]
    })
    cleaned = analysis.calculate_technical_indicators()
    
    assert "SMA" in cleaned.columns
    assert "EMA" in cleaned.columns
    assert "RSI" in cleaned.columns
    assert "MACD" in cleaned.columns
    assert "MACD_SIGNAL" in cleaned.columns
